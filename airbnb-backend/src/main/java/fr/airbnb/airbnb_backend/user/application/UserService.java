package fr.airbnb.airbnb_backend.user.application;
...
import fr.airbnb.airbnb_backend.infrastructure.config.SecurityUtils;
import fr.airbnb.airbnb_backend.user.application.dto.ReadUserDTO;
import fr.airbnb.airbnb_backend.user.domain.User;
import fr.airbnb.airbnb_backend.user.mapper.UserMapper;
import fr.airbnb.airbnb_backend.user.repository.UserRepository;
...
public class UserService {
    ...
    public void syncWithIdp(OAuth2User oAuth2User, boolean forceResync) {
        ...
        if (existingUser.isPresent()) {
            if (attributes.get(UPDATED_AT_KEY) != null) {
                Instant lastModifiedData = existingUser.orElseThrow().getLastModifiedDate();
                Instant idpModifiedData;
                if (attributes.get(UPDATED_AT_KEY) instanceof Instant instant) {
                    idpModifiedData = instant;
                } else {
                    idpModifiedData = Instant.ofEpochSecond((Integer) attributes.get(UPDATED_AT_KEY));
                }
                if (idpModifiedData.isAfter(lastModifiedData) || forceResync) {
                    updateUser(user);
                }
            }
        } else {
    ...
    public Optional<ReadUserDTO> oneByPublicId (UUID publicId) {
