...
public class SecurityUtils {
    ...
    public static User mapOauth2AttributesToUser(Map<String, Object> attributes) {
        ...
        if (attributes.get("picture") != null) {
            user.setImage_url((String) attributes.get("picture"));
        }
    ...
    private static Stream<String> getAuthorities(Authentication authentication) {
        Collection<? extends GrantedAuthority> authorities = authentication instanceof JwtAuthenticationToken jwtAuthenticationToken ? extractAuthorityFromClaims(jwtAuthenticationToken.getToken().getClaims()) : authentication.getAuthorities();
